...
RUN CGO_ENABLED=0 GOOS=linux go build -o /grpc-device-logging
...
COPY --from=build-stage /grpc-device-logging /grpc-device-logging
...
ENTRYPOINT ["/grpc-device-logging"]